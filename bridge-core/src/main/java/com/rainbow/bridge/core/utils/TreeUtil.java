...
public class TreeUtil {
    ...
    /**
     * 集合转树结构
     * 案例：List<Dept> newDeptList = listTreeNodes("deptId", "pid", "children", deptList, Dept.class);
     *
     ...
     * 使用方法
     * BaseTreeUtils.listTreeNodes("name","parentId","children",routeVos,RouteVo.class);
     *
     * @param allList  目标集合
     * @param id       节点编号字段名称
     ...
     * @return 转换后的树形结构
     */
    public static <T> List<T> listTreeNodes(String id, String parent, String children, List<T> allList, Class<T> clazz) {
        // 如果被依赖字段名称为空则默认为id
        ...
        try {
            ...
            // 遍历根节点, 依次添加子节点
            for (T root : roots) {
                addChild(root, allList, idField, parentField, childrenField);
            }
    ...
    private static boolean isRootNode(Object parentId) {
        ...
        if (parentId == null) {
            flag = true;
        } else if (parentId instanceof String && (StringUtils.isEmpty(parentId.toString()) || parentId.equals("0"))) {
            flag = true;

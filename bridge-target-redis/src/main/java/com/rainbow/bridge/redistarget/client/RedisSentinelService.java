...
import com.rainbow.bridge.redistarget.client.prop.RedisSentinelProperties;
 ...
 * @author gujiachun
 */
public class RedisSentinelService implements RedisService {

    public static final Logger logger = LoggerFactory.getLogger(RedisSentinelService.class);
    ...
    public RedisSentinelService(RedisSentinelProperties properties) {
        ...
        if (!StringUtils.isEmpty(properties.getNodes())) {
            // 以“；”分割成"ip:post"
            String[] ipAndPostes =  properties.getNodes().split(","); //StringUtils.split(properties.getNodes(), ",");
            // 以“：”分割成 “ip”，“post”
            if (ipAndPostes != null) {
        ...
        logger.info("redis链接节点个数(sentinelNodes)：{}", nodes.size());
        ...
        jedisSentinelPool = new JedisSentinelPool(properties.getMasterName(), nodes,jedisPoolConfig,
                properties.getConnectTimeout(), properties.getSoTimeout(), properties.getPassword(),properties.getDatabase());

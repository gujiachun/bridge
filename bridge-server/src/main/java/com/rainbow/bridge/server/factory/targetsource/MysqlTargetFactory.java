 ...
 * @author gujiachun
 */
public class MysqlTargetFactory extends AbsTargetFactory {
    ...
    protected Object initTargetConn(TargetConn targetConn) {
        ...
        try {
            ...
            logger.info("初始DataSource成功,目标数据源:{}",targetConn.getTargetId());
            return ds;
        } catch (Exception e) {
            logger.error("初始DataSource发生异常，目标数据源:{},异常:{}",targetConn.getTargetId(),e.getMessage());
        }
        return null;
    }
    ...
    protected List<Integer> getTargetsByTaskId(String taskId) {
        ...
        for (SyncTaskRuleMysqlEntity syncTaskRuleMysqlEntity : syncTaskRuleMysqlEntityList){
            ...
            if (!targetIds.contains(syncTaskRuleMysqlEntity.getTargetId())){
                ...
                logger.info("任务id:{},需要同步到 目标源targetId:{}",taskId,syncTaskRuleMysqlEntity.getTargetId());
            }
        }
